...
let CoursService = class CoursService {
    ...
    async findCours(id_cours) {
        const cours = await this.coursRepository.findOneBy({ id_cours });
        if (!cours)
            throw new common_1.NotFoundException(`le cours ${id_cours} n'a pas été trouvé`);
        return cours;
    }
    async createCours(cours, file) {
