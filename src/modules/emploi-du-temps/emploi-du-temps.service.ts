...
export class EmploiDuTempsService {
  ...
  async create(createDto: CreateEmploiDuTempsDto): Promise<EmploiDuTemps> {
    ...
    const conflit = await this.emploiRepo
      ...
      .andWhere(
        "(emploi.heureDebut < :heureFin AND emploi.heureFin > :heureDebut)",
        {
  ...
  async update(
    id: number,
    updateDto: UpdateEmploiDuTempsDto,
    ...
    if (updateDto.date || updateDto.heureDebut || updateDto.heureFin) {
      ...
      if (conflit) {
        throw new BadRequestException(
          "Conflit de planning pour ce professeur.",
        );
      }
    }
    ...
    if (updateDto.professeur) {
      updateDto = {
        ...updateDto,
        professeur: { id: updateDto.professeur } as any,
      };
    }
